...
const materialOptions = ["PREMIUM GENUINE LEATHER", "SYNTHETIC (PU) LEATHER", "SHOCK-ABSORBING EVA FOAM"];
const brandingOptions = ["SCREEN PRINT", "EMBROIDERY", "3D RUBBER PATCH / WOVEN LABEL"];
const closureOptions = ["PRINTED LOGO ON VELCRO STRAP", "VELCRO CLOSURE STRAP", "LACE-UP"];
const packagingOptions = ["POLY BAG", "ZIP PE BAG", "CUSTOM DRAWSTRING BAG"];
  ...
  return (
    <div className="min-h-screen bg-white font-['FFF_Acid_Grotesk',sans-serif]">
      ...
      <section className="relative h-[320px] md:h-[480px] overflow-hidden flex justify-center">
        <div className="absolute inset-0 bg-black max-w-[2560px] w-full mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-32 max-w-[2560px] mx-auto lg:justify-center">
          ...
          <div className="w-full lg:w-[616px] shrink-0">
            <div className="bg-[#F9F9F9] rounded-none p-4 md:p-2 aspect-[4/5] md:aspect-square flex items-center justify-center lg:sticky lg:top-24 max-w-[800px] mx-auto">
              <img src="/Products/07 Custom Boxing Headguards.png" alt="Boxing Head Guard" className="w-[100%] h-[100%] md:w-[600px] md:h-[600px] object-cover md:object-contain mix-blend-multiply" />
            </div>
          </div>
          ...
          <div className="flex-1 max-w-full lg:max-w-[616px] mx-auto lg:mx-0">
            ...
            <OptionGroup label="Closure" guideLabel="Closure Guide" options={closureOptions} selectedOptions={selectedClosure} onSelect={(val) => toggleOption(val, selectedClosure, setSelectedClosure)} />
            ...
            <div className="mb-6 md:mb-8">
              <div className="flex items-center justify-between mb-3">
                ...
                <button className="text-[12px] leading-[16px] tracking-[0px] font-normal text-[#666666] hover:underline text-left md:font-medium md:text-xs">Size Guide</button>
              </div>
              <div className="space-y-4">
                <div className="flex flex-wrap gap-2 md:gap-3">
                  {sizeOptions.map((size) => (
                    <button key={size} onClick={() => toggleOption(size, selectedSize, setSelectedSize)} className={`px-4 py-3 md:px-5 md:py-[15px] border text-[15px] leading-[17px] tracking-[0.4px] font-medium text-left transition-all md:tracking-normal ${selectedSize.includes(size) ? "border-[#0D0D0D] text-[#0D0D0D] bg-white" : "border-gray-200 text-[#0D0D0D] hover:border-gray-400"}`}>
                      {size}
                    </button>
                  ))}
                </div>
                <div>
                  <span className="block text-[12px] leading-[16px] tracking-[0px] font-normal text-[#666666] mb-2 uppercase text-left md:font-medium md:text-xs">Other</span>
                  <input type="text" placeholder="Any other" className="border border-gray-200 px-4 py-3 md:py-[15px] w-full text-[15px] leading-[17px] tracking-[0.4px] font-medium focus:outline-none focus:border-[#0D0D0D] md:text-[15px] md:tracking-normal" />
                </div>
              </div>
            </div>
            ...
            <div className="mb-6 md:mb-8">
              <div className="flex items-center justify-between mb-3">
                <label className="text-[14px] leading-[18px] tracking-[0.1px] font-bold text-[#0D0D0D] uppercase text-left md:text-sm md:tracking-normal">Quantity</label>
              </div>
              <input type="number" placeholder="Enter quantity" className="border border-gray-200 px-4 py-3 md:py-[15px] w-full text-[15px] leading-[17px] tracking-[0.4px] font-medium focus:outline-none focus:border-[#0D0D0D] md:text-[15px] md:tracking-normal" />
              <div className="flex items-center gap-2 mt-3">
                <span className="text-[#D92D20] shrink-0"><InfoIcon /></span>
                <span className="text-[#D92D20] text-[12px] leading-[16px] font-medium tracking-wide md:text-[12px]">Please ensure your order meets our Minimum Quantity</span>
              </div>
            </div>
  ...
  return (
    <div className="mb-6 md:mb-8">
      <div className="flex items-center justify-between mb-3">
        ...
        {guideLabel && <button className="text-[12px] leading-[16px] tracking-[0px] font-normal text-[#666666] hover:underline text-left md:font-medium md:text-xs">{guideLabel}</button>}
      </div>
      <div className="flex flex-wrap gap-2 md:gap-3">
        {options.map((opt) => (
          <button key={opt} onClick={() => onSelect(opt)} className={`px-4 py-3 md:px-5 md:py-[15px] border text-[15px] leading-[17px] tracking-[0.4px] font-medium text-left transition-all md:tracking-normal ${selectedOptions.includes(opt) ? "border-[#0D0D0D] text-[#0D0D0D] bg-white" : "border-gray-200 text-[#0D0D0D] hover:border-gray-400"}`}>
            {opt}
          </button>
        ))}
      </div>
...
function FooterAccordion({ title, children }: { title: string; children: React.ReactNode; }) {
  ...
  return (
    <div className="border-b border-gray-200">
      <button className="w-full flex items-center justify-between py-4 text-left text-[14px] leading-[17px] tracking-[0px] font-normal text-[#0D0D0D] md:font-medium" onClick={() => setOpen(!open)}>
        {title}
...
function FooterLink({ href, children }: { href: string; children: React.ReactNode; }) {
  return <a href={href} className="hover:text-[#0D0D0D] transition block text-[14px] leading-[17px] tracking-[0px] font-normal text-[#434343] text-left md:text-[#757575]">{children}</a>;
}